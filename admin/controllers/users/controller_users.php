...
elseif($module[3] == "edit")
{
    if((int)$module[5] > 0)
    {
        one_user_info::init($db);
        if($module[4] == "students")
        {
            if(Root::POSTExists("add"))
            {
                if(!one_user_info::changeStudentInfo((int)$module[5]))
                    $View->error_pass = true;
            }
            $user_info = one_user_info::getUserInfo((int)$module[5],"S");
            require_once("admin/classes/groups/class.groups.php");
            $groups = new groups($db);
            $groups->getGroups();
        }
        elseif($module[4] == "lectors")
        {
            if(Root::POSTExists("add"))
            {
                if(!one_user_info::changeLectorInfo((int)$module[5]))
                    $View->error_pass = true;
            }

            $user_info = one_user_info::getUserInfo((int)$module[5],"P");
            require_once("admin/classes/subjects/class.subjects.php");
            $subjects = new subjects($db);
            $subjects->getSubjects();

            $View->right_subjects = explode("||",$user_info['Subjects']);
            $View->left_subjects = array_diff($subjects->getSubejctsIds(),$View->right_subjects);
        }

        $View->sub_module = "edit_user";
    }
}
?>
