...
//    }
    ...
    public function getMagic()
    {
        ...
        $sql = "SELECT `ID`,`Title_ua` FROM `Subjects` WHERE ";
        ...
        foreach($res as $subject_id=>&$some){
            foreach($s_temp as $two){
                if($subject_id==$two["ID"]) $some["subject_title"]=$two["Title_ua"];
            }
    ...
    public function getMyTables(){
        $sql="SELECT r.`Tablename` AS `tablename`,s.`Title_ua` AS `subject`,g.`Title` AS `group` FROM `Ratings` AS r, `Subjects` AS s, `Groups` AS g WHERE ";
...
// хз че там дальше))
    ...
    public function getMyGroups()
    {
        ...
        $sql = "select `Title_ua` from `Subjects` where ";
