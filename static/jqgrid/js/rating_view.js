    ...
    $.ajax({
        ...
        success:function(data)
        {
            ...
            if (!table){
                ...
                $.extend(opts,{ caption:"Оценки группы: "+$("#group option:selected").text()+" / "+$("#subject option:selected").text() });
                ...
                if(foodata["ratio"]!="0"){foodata["ratio"]="1.00";}
                ...
                for(i = 0; i < table["data"].length-1; i++){
                    ...
                    $("#desttable tr:nth-child("+(i+1)+") td:nth-child("+(colmod.length+1)+")").css("background", "#EFEFEF");
...
function updateSelect(table_select,key,group_id){
    var html='<option>...<\/option>';    
    if (key=="subject") { html+='<option>'+js_labels["all"]+'<\/option>'; }
    if (table_select) {
        for (var i in table_select) {
            var c=table_select[i]["tablename"];
